...
mod tests {
    ...
    fn can_get_taxonomy() {
        ...
        let items = res_obj.get(&Key::from("items")).unwrap().as_array().unwrap();
        assert_eq!(items.len(), 1);
        let item = items[0].as_map().unwrap();
        assert_eq!(item.get(&Key::from("name")).unwrap().as_str().unwrap(), "Programming");
        assert_eq!(item.get(&Key::from("slug")).unwrap().as_str().unwrap(), "programming");
        ...
        assert_eq!(items.len(), 1);
        let item = items[0].as_map().unwrap();
        assert_eq!(item.get(&Key::from("name")).unwrap().as_str().unwrap(), "Programmation");
    ...
    fn can_get_taxonomy_term() {
        ...
        let res_obj = res.as_map().unwrap();
        assert_eq!(res_obj.get(&Key::from("name")).unwrap().as_str().unwrap(), "Programming");
        assert_eq!(res_obj.get(&Key::from("slug")).unwrap().as_str().unwrap(), "programming");
        ...
        let res = get_taxonomy_term.call(kwargs, &State::new(&ctx)).unwrap();
        let res_obj = res.as_map().unwrap();
        assert_eq!(res_obj.get(&Key::from("name")).unwrap().as_str().unwrap(), "Programmation");
